...
[参考let,const](https://www.cnblogs.com/polk6/p/js-letAndconst.html)

ECMAScript6增加`let`和`const`，用来更精确地声明作用域和语义。**`const`优先，`let`次之，最好不使用`var`。**
...
null值表示一个空对象指针，`typeof`返回”Object“。
...
布尔值是使用最频繁的类型之一，有两个字面值：true和false。
...
|=|===========|===================================================|
|s|-exponent--|--------------------mantissa-----------------------|
//上面的图示中，s(sign)为符号位，占1bit，用来表示整个float/double的正负性；中间部分exponent是科学计数法指数部分，即阶码；最后的也是最长的一部分mantissa，尾数，它的长度直接影响力浮点数的精度。
``````
